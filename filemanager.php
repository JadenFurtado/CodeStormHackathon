...
session_start();
//configure the root path of the code directory
$rootPath="http://localhost/code_editor_1/";
/**
 * H3K | Tiny File Manager V2.4.6
 * CCP Programmers | ccpprogrammers@gmail.com
 * https://tinyfilemanager.github.io
 */
//function to send session request to the editor
...
"user"=>$_SERVER['DOCUMENT_ROOT']."/code_editor/code/batch_1/"
);
//add more users
while($row=mysqli_fetch_array($res)){
    $auth_users[$row['student_name']] = $row['student_password'];   
    $directories_users[$row['student_name']]=$_SERVER['DOCUMENT_ROOT']."/code_editor/code/".$row['instructor_id']."/".$row['student_name'];
    $users_instructor[$row['student_name']]=$row['instructor_id'];
}
// Readonly users
...
$root_path = $_SERVER['DOCUMENT_ROOT']."/code_editor/code/";
...
//to log users activity
$link = new mysqli("localhost","root","","code_editor");
    $sql ="SELECT id FROM students WHERE student_name = '".$_POST['fm_usr']."'";
    //echo $link->error;
    $res = mysqli_query($link,$sql);
    //print_r($res);
    while($row = mysqli_fetch_array($res)){
    ...
    $sql = "INSERT INTO login_history(student_id,session_id) VALUES('$student_id','".$_COOKIE['filemanager']."')";
...
if ($use_auth) {
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        if(function_exists('password_verify')) {
            //need to remove certain parts in order to use a db for loggin in 
            if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']])) {
                $_SESSION[FM_SESSION_ID]['logged'] = $_POST['fm_usr'];

                $link = new mysqli("localhost","root","","code_editor");
                $sql = "INSERT INTO student_login_history(student_name,session_id) VALUES('".$_SESSION['filemanager']['logged']."','".$_COOKIE['filemanager']."')";
                mysqli_query($link,$sql);
                                ...
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="form-group">
                                       <div class="brand">
                                        ...
                                        -->
                                        <img src="astringe logo.png">
                                        </div>
                                        <div class="text-center">
                                            <h1 class="card-title"><?php echo 'Astringe Infosystems' ?></h1>
                                        </div>
                                    </div>
                                    <hr />
                                    <div class="form-group">
            ...
            <?php if(!$quickView) { ?>
                ...
                <p>
                    ...
                    if ($is_text && !FM_READONLY) {
                        ?>
                        <b><a href="https://localhost/code_editor/test2.php?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo lng('Edit') ?>
                            </a></b> &nbsp;
                            <!--!!!!!!!!!! part to be removed !!!!!!-->
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php
                           ...
                           <?php else: ?>
                            <?php if(FM_PATH==NULL): ?>
                                <a href="<?php echo $rootPath;  ?>public/Editor?p=&edit=<?php echo '/'.$users_instructor[$_SESSION[FM_SESSION_ID]['logged']].'/'.$_SESSION[FM_SESSION_ID]['logged'].'/'.$f."&file_name=".$f ?>" title="<?php echo $f ?>">
                                <?php else: ?>
                                    <a href="<?php echo $rootPath; ?>public/Editor?p=&edit=<?php echo '/'.$users_instructor[$_SESSION[FM_SESSION_ID]['logged']].'/'.$_SESSION[FM_SESSION_ID]['logged'].'/'.FM_PATH.'/'.$f."&file_name=".$f  ?>" title="<?php echo $f ?>">
                                <?php endif; ?>
                            <?php endif; ?>
                    ...
                    <td class="inline-actions">
                        ...
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL .'?edit='. (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="fa fa-link"></i></a>
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <a class="navbar-brand" href=""> <img src="https://localhost/code_editor_1/astringe logo.png" style="max-height:60px;"> </a>
...
<img src="https://localhost/code_editor_1/astringe logo.png">
<div id="wrapper" class="container-fluid">
